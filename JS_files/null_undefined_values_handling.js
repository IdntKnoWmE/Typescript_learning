...
function Example1() {
    ...
    let tempNumber = null; // Here, intentionally temp number is marked empty so, TS throw no error.
}
...
function Example6() {
    ...
    function winLottery(lotteryNumber) {
        const number = Math.floor(Math.random() * 10);
        if (number === lotteryNumber) {
            return "Hooray! You won";
        }
        if (number + 1 === lotteryNumber) {
            return null;
        }
    }
