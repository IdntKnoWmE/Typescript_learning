...
function welcomeMessage(name: string, title?: string) : string{
    if (title){
...
function printSomething(something: string = "Nothing is here, bitch!") : string{
    return something;
}
...
function getTime(){ // TS will automatically guess the return type as number based on return statement
...
function createUserBoy(username: string, age: number, isAdmin: boolean, isActive: boolean) {}
createUserBoy("JohnDoe", 30, true, false);

// - The Solution: Named Parameters
//   By wrapping your arguments into an object type and destructuring them, you create named parameters.
//   Here is a comprehensive breakdown of how to declare, type, and initialize named parameters in TypeScript.

// Define an interface or type alias for the parameters
interface createUserInterface{
    username: string;
    age: number;
    isActive?: boolean; // optional parameter
    role?: string; // optional parameter
}
// Implement the function using object destructuring and the type
function CreateUserBabe({username, age, isActive=true, role="user"}:createUserInterface){
...
function printCord({x,y }: {x: number; y: number}) {
    console.log(`X: ${x}, Y: ${y}`);
}
printCord({x: 1, y: 1}); // Always a objects will be passed as parameter with all parameters inside

// 2) Making the Entire Object Optional
//

interface themeConfig{
    theme?: "dark" | "light";
    fontStyle?: "normal"| "arial"| "italic";
}
function setTheme({theme="light", fontStyle="normal"}:themeConfig) {
...
function setTheme2({theme="light", fontStyle="normal"}:themeConfig = {}) {
    console.log(`Set Browser theme: ${theme} and font style as ${fontStyle}`);
}
setTheme2(); // Now you call the function like this without error.

// 3) Renaming Variables During Destructuring
//  Sometimes you want the external API option to have one name, but use a cleaner local variable name inside your
//  function code. You can achieve this using JavaScript destructuring syntax:

interface RedirectConfig {
    url: string;
    sec: number; // 'sec' is what the caller provides
}

function startTimeout({ url, sec: durationInSeconds }: RedirectConfig) {
...
function myFunc(...args: [string, number]){
    console.log(args);
}
myFunc("abcd", 23); // Works fine
// myFunc("abcf"); // Will throw: Expected 2 arguments, but got
// myFunc("df",3,234); // Will throw: Expected 2 arguments, but got 3.


// 2) Optional and Variadic Tuple Elements
//  You can combine rest parameters with advanced tuple types to make some trailing arguments optional or
//  variable in length.

// The first element is required, the second is optional, and the rest can be strings
type Arguments = [string, number?, ...string[]];

function doExercise(...args: Arguments){
...
const negateFunc: negate = (value: number) => value * -1;
