...
function basic_promise_example(){
    ...
    fetchUserProfile().then(
        (user: UserProfile) => {console.log("basic_promise_example output: ", user, "\n")}
    )
        .catch((error: unknown)=> {
...
function basic_async_await_example(){
    ...
    async function getCurrentUserRole(): Promise<keyof Role | undefined> {
        try {
            const user: UserProfile = await fetchCurrentUserProfile();
            return user.role;
        } catch (error) {
    ...
    getCurrentUserRole().then(
        (role) => console.log(`basic_async_await_example output: The role of current user is ${role}\n`)
    ).catch(
        (error: Error) => console.error(`basic_async_await_example output: Failed to fetch current user role: ${error.message} \n`)
    )
}
...
function calling_real_api(){
    ...
    async function fetchPostDetail(id: number): Promise<PostDetail> {

        try {
            let postResponse: Response = await fetch(`https://jsonplaceholder.typicode.com/posts/${id}`);
            if (postResponse.ok) {
                const data = await postResponse.json() as PostDetail;
                return data;
            } else {
    ...
    let id = 1;
    fetchPostDetail(id).then(
        (postDetail) => console.log(`Details of post Id: ${id} is: ${JSON.stringify(postDetail)}`)
    ).catch(
        (error: Error) => console.error('Failed to fetch post detail: ', error.message)
    )
}
calling_real_api();
...
function multiple_parallel_promise_example() {
    ...
    async function fetchMultipleProduct(ids: number[]){
    ...
    let productIds: number[] = [1, 2, 3, 4, 5];
    fetchMultipleProduct(productIds).then(
        (totalPrice) => console.log(`multiple_parallel_promise_example output: The total price for products ${productIds} is $${totalPrice} \n`)
    ).catch(
        (error: Error) => console.error('multiple_parallel_promise_example output: ', error.message, '\n')
    )
}
...
function promise_all_example() {
    ...
    const fetchUser = (id: number): Promise<UserProfileType> => new Promise((resolve) => {
        return setTimeout(() => {
            resolve({id: id, name: "John Doe"});
        }, 500)
    })

    type UserPostsType = Array<{
        id: number
        detail: string
    }>

    const fetchPosts = (userId: number): Promise<UserPostsType> => new Promise((resolve) => {
        return setTimeout(() => {
    ...
    const fetchStats = (userId: number): Promise<UserStatsType> =>
...
function promise_race_example() {
    ...
    function randomReject(time: number): Promise<never> {
    ...
    function randomResolve(time: number): Promise<String> {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
    ...
    async function callRandom(): Promise<void> {

        const time1: number = Math.floor(Math.random() * 5) * 500;
        const time2: number = Math.floor(Math.random() * 5) * 500;
        try {
...
function promise_allsettled_example() {
    ...
    function fetchData(id: number): Promise<ApiDataType> {
    ...
    async function fetchBulkData(ids: number[]): Promise<void>{
...
function custom_error_class_example() {
    ...
    class AppError extends Error {
        constructor(message: string,
                    public readonly code: string, // Since publics is used then TS will creates an internal class variable.
                    public readonly detail: unknown) // Same here
        {
            super(message);
            this.name = this.constructor.name;
            (Error as any).captureStackTrace?.(this, this.constructor); // It removes the constructor name from the stack trace.
        }
    }
    ...
    function mockUserAPI(url: string, id: string) {
    ...
    async function fetchUserData(userId: string): Promise<UserData> {
        try {
            const response: any = await mockUserAPI("http://localhost:8080", userId)

            if (!response.ok) {
                if (response.status === 404) throw new NotFoundError('User does not exist', userId);
                else if (response.status === 500) throw new NetworkError('Network Error', userId);
