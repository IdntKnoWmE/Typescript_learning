...
function Example1(){
    ...
    const user1: UserDetails = {id: 123, name: "john", email: "abcd@gmail.com", };
    ...
    const user2: Partial<UserDetails> = {id: 124}
}
...
function Example2(){
    ...
    const user1: UserDetails = {id: 123}; // This Works absolutely fine!
    const user2: UserDetails = {id: 124, name: "Johnny"}; // This also works fine!

    // 2. With Required, every single property of UserDetails is required now.
    const user3: Required<UserDetails> = {id: 124, name: "Johnny", email: "johnny@abc.com"}; // Works fine!
...
function Example3(){
    ...
    const user1: UserDetails = {id: 123, name: "Jayant", email: "jj@gmail.com"};
    user1.name = "Jayant The Best of the Best"; // Works absolutely fine!
    console.log(user1);

    // With readonly, you cannot change the value of any field now.
    const user2: Readonly<UserDetails> = {id: 123, name: "Jayant The Billionaire", email: "jj@gmail.com"};
...
function Example4(){
    ...
    const checkout2: Pick<Product, "name" | "price"> = {
        name: "Condom",
        price: 14,
...
function Example5(){
    ...
    const checkout2: Omit<Product, "description"> = {
        id: 1,
        name: "Condom",
        price: 14,
        units: 10,
    ...
    const checkout3: Omit<Product, "description" | "units" | "id"> = {
        name: "Condom",
        price: 14,
    } // Works perfectly!
}
...
function Example7(){
    type AllStatuses = "success" | "pending" | "error" | "idle";

    // Filter out 'idle' and 'pending' from the union
    type ActiveStatuses = Exclude<AllStatuses, "idle"|"pending">;

    // Now ActiveStatuses is exactly: "success" | "error"
    let currStatus: ActiveStatuses = "success"; // Works fine
...
function Example8(){
    ...
    // Pulls out 'success' and 'error' from the union
    type ActiveStatuses = Extract<AllStatuses, "success"|"error">;
...
function Example9(){
    ...
    // TypeScript looks inside the function and extracts the shape of the returned object
    type UserDetailReturnType = ReturnType<typeof userDetails>;
    // Now you could do anything with this detail like create a new user with type same as return type of
    // userDetails function.

    const user: UserDetailReturnType = {
        id: 10,
        name: "Johnny",
