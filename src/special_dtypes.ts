...
let mysteryItem: any = "Hello World";
...
let mysteryItem2 : unknown = "Hello World";
...
function throwsError(err: string) : never {
    throw new Error(err);
}
...
// Now, You write a function to calc area. You use never as a safety net at the very bottom to
// catch anything unexpected.

type Shape = Circle | Square ; // Assigning user defined dtype to our Shape variable.
...
function getArea(shape: Shape): number {
    switch (shape.kind) {
        ...
        default:
            ...
            // @ts-ignore (Remove it)
            const fallback: never = shape;
            return fallback;
    }
}
// The Mistake (Where never Saves You)
// Suppose months later, you want to calc area for triangle also. You update your type definition:

// type newShape = Circle | Square | Triangle;

// Here when Triangle is assigned to type the function skips all the cases and execute default case, In default
// case we have fallback assigned to type never but we assign a shape here, so TypeScript will throw the error
// to our newShape assignment only so, we will know that a new shape still not added in the function.


// c) An Impossible Intersection
// If you try to combine two incompatible types, TypeScript will resolve the result to never because a value
